...
class KDTree {
  private:
    /** Internal structure for a node of the KDTree. Contains left, right children pointers and an airport. */
    struct KDTreeNode {
      Airport airport;
      KDTreeNode* left, *right;

      KDTreeNode() : left(nullptr), right(nullptr) {}
      KDTreeNode(const Airport &airport) : airport(airport), left(nullptr), right(nullptr) {}
    };

  public:
    /**
     * Determines if an airport A is smaller than airport B with given coordinates in a given dimension.
     ...
     * @return boolean value indicating whether the first airport is smaller than the second airport in the given dimension
     */
    bool smallerDimVal(double firstLat, double secondLat, double firstLong, double secondLong, 
                       int curDim) const;

    /**
     * Determines if an airport (currBest) is closer to the target airport than another
     ...
     * @return boolean value indicating whether potential is closer to target than currentBest
     */
  ...
  private:
    ...
    /** Computes the great-circle distance between two airports using their associated coordinates. */
