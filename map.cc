...
void Map::AddPoint(double lat, double lng, double luminance) {
  ...
  for (int i = max(0, (int)x - 1); i < min((int)map_.width(), (int)x + 1);
       ++i) {
    for (int j = max(0, (int)y - 1); j < min((int)map_.height(), (int)y + 1);
         ++j) {
...
void Map::Animate(const FlightGraph& graph, const AirportMap& airports,
  ...
  while (!queue.empty()) {
    ...
    AddPoint(airports.at(curr).getLatitude(), airports.at(curr).getLongitude(),
             fmin(0.25 + (last_seen_degree + 1) * 0.15, 1.0));
...
cs225::PNG& Map::GetMap() { return map_; }